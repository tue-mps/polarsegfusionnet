...
class FPN_BackBone_radar(nn.Module):
    ...
    def __init__(self, num_block, channels, channels_bev, block_expansion, mimo_layer, use_bn=True):
        ...
        self.in_planes = 128 + 64  # (128 comes from block1 of radar and 64 comes from block1 of camera)
    ...
    def forward(self, x, bev_inputs):
        ...
        features_radar['x1_fused'] = torch.cat((features_radar['x1'], features_camera['x1']), axis=1)

        x2 = self.block2(features_radar['x1_fused'])
        x3 = self.block3(x2)
        x4 = self.block4(x3)
