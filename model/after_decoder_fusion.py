...
class PolarSegFusionNet_RA(nn.Module):
    ...
    def forward(self, x):
        ...
        fusion_list_radar.append(RA_decoded)

        return fusion_list_radar
...
class SegmentationHead_afterdec(nn.Module):
    def __init__(self, mimo_layer, channels, channels_bev, blocks, segmentation_head, radar_input, camera_input, fusion):
        ...
        if self.fusion and self.segmentation_head:
            ...
            self.freespace_fusion = nn.Sequential(BasicBlock(384, 128),
                                                 BasicBlock(128, 64),
    ...
    def forward(self, ra_inputs, bev_inputs):
        ...
        if self.fusion and self.segmentation_head:
            fusion_list_radar = self.radarenc(ra_inputs)

            fusion_list_camera = self.cameraenc(bev_inputs)

            out_fused = torch.cat((fusion_list_radar[0], fusion_list_camera[0]), axis=1)

            R = F.interpolate(out_fused, (256, 224))
