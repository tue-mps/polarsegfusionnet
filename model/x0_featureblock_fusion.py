...
class FPN_BackBone_radar(nn.Module):
    ...
    def __init__(self, num_block, channels, channels_bev, block_expansion, mimo_layer, use_bn=True):
        ...
        #pre-encoder feature of camera input
        self.cameraenc = PolarSegFusionNet_BEV(channels_bev=channels_bev)

        # Residuall blocks
        self.in_planes = mimo_layer + 32  # (32 comes from the warmup block of FPN camera encoder)
    ...
    def forward(self, x, bev_inputs):
        ...
        features_radar = {}
        features_radar['x0'] = x
        features_radar['x0_fused'] = torch.cat((features_radar['x0'], features_camera['x0']), axis=1)
        x1 = self.block1(features_radar['x0_fused'])
        x2 = self.block2(x1)
        x3 = self.block3(x2)
        x4 = self.block4(x3)
...
class RangeAngle_Decoder(nn.Module):
    def __init__(self, ):
        ...
        # self.deconv2 = nn.ConvTranspose2d(64, 64, kernel_size=3, stride=(4, 1), padding=1, output_padding=(3, 0))
        # self.conv_block2 = BasicBlock(320, 256)

        self.L3 = nn.Conv2d(192, 224, kernel_size=1, stride=1, padding=0)
        self.L2 = nn.Conv2d(160, 224, kernel_size=1, stride=1, padding=0)
        # self.L1 = nn.Conv2d(512, 64, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, features_radar):
        ...
        T0 = features_radar['x0_fused'].transpose(1, 3)
...
class PolarSegFusionNet_RA(nn.Module):
    def __init__(self, mimo_layer, channels, channels_bev, blocks):
        ...
        self.FPN = FPN_BackBone_radar(num_block=blocks, channels=channels, channels_bev=channels_bev, block_expansion=4, mimo_layer=mimo_layer,
                                      use_bn=True)

    def forward(self, x, bev_inputs):
        features_radar = self.FPN(x, bev_inputs)
        return features_radar
...
class FPN_BackBone_camera(nn.Module):
    ...
    def __init__(self, block_expansion, use_bn=True):
...
class PolarSegFusionNet_BEV(nn.Module):
    def __init__(self, channels_bev):
        super(PolarSegFusionNet_BEV, self).__init__()
        self.FPN = FPN_BackBone_camera(block_expansion=4, use_bn=True)
...
class SegmentationHead_x0(nn.Module):
    def __init__(self, mimo_layer, channels, channels_bev, blocks, segmentation_head, radar_input, camera_input, fusion):
        super(SegmentationHead_x0, self).__init__()

        self.segmentation_head = segmentation_head
        self.radar_input = radar_input
        self.camera_input = camera_input
        self.fusion = fusion

        if self.fusion and self.segmentation_head:
            ...
            self.freespace_fusion = nn.Sequential(BasicBlock(256, 128),
                                                 BasicBlock(128, 64),
                                                 nn.Conv2d(64, 1, kernel_size=1))

    def forward(self, ra_inputs, bev_inputs):
        ...
        if self.fusion and self.segmentation_head:
            ...
            R = F.interpolate(RA_decoded, (256, 224))

            out['Segmentation'] = self.freespace_fusion(R)
