...
def RADIal_collate(batch):
    ...
    for radar_FFT, segmap, bev_image, cameraimage, cam_img_name in batch:
        ...
        cameraimages.append(torch.tensor(cameraimage).permute(2, 0, 1))

    return torch.stack(FFTs), torch.stack(segmaps), torch.stack(bev_images), torch.stack(cameraimages)
...
def CreateDataLoaders(dataset,config=None,seed=0):
    ...
    if(config['mode']=='random'):
        ...
        for seq in Sequences['Validation']:
            idx = np.where(dataset.sampleid[:,14]==seq)[0]
            Val_indexes.append(dataset.sampleid[idx,0])
        Val_indexes = np.unique(np.concatenate(Val_indexes))

        Test_indexes = []
        for seq in Sequences['Test']:
            idx = np.where(dataset.sampleid[:,14]==seq)[0]
            Test_indexes.append(dataset.sampleid[idx,0])
